...
export default function Header() {
    ...
    const userMenu = useAuthStore((s) => s.userMenu);
    ...
    const userIcon = useMemo(() => {
        switch (user?.role) {
            case 'admin': return 'crown' as const;
            case 'listener': return 'headphones' as const;
            case 'creator': return 'music' as const;
                        ...
                        <Nav className="d-flex flex-row align-items-center gap-2 gap-lg-3 ms-auto">
                            ...
                            {isLogged ? (
                                <>
                                    {hasNotifications && (
                                        <Dropdown>
                                            <Dropdown.Toggle variant="" className="btn-default p-2 btn-icon" id="notif-dropdown">
                                                <FontAwesomeIcon icon={['fas', 'bell']} />
                                                <span className="badge bg-danger rounded-pill">{unreadNotifications.length}</span>
                                            </Dropdown.Toggle>
                                            <Dropdown.Menu>
                                                {unreadNotifications.map((n: any) => (
                                                    <Dropdown.Item key={n.id}>{n.data?.message || 'Notification'}</Dropdown.Item>
                                                ))}
                                            </Dropdown.Menu>
                                        </Dropdown>
                                    )}
                                    <Dropdown>
                                        <Dropdown.Toggle variant="" className="btn-default p-2 btn-icon" id="user-dropdown">
                                            {user?.artwork ? (
                                                <img src={user.artwork} alt={user.name} className="rounded-circle" width={32} height={32} />
                                            ) : (
                                        ...
                                        <Dropdown.Menu>
                                            {user && (
                                                <Dropdown.Item as={Link} to={`/user/${user.username}`}>
                                                    <FontAwesomeIcon icon={['fas', userIcon]} className="me-2" />
                                                    {user.name}
                                                </Dropdown.Item>
                                            )}
                                            <Dropdown.Divider />
                                            ...
                                            <Dropdown.Divider />
                                            <Dropdown.Item onClick={handleLogout}>{$t('menus.logout')}</Dropdown.Item>
                                        </Dropdown.Menu>
                                    </Dropdown>
                                </>
                            ) : (
