            ...
            <div class="checkout-container">
                ...
                @foreach($cart as $item)
                    <div class="src-mainapp-pages-trackshop-___TrackTable__track___1KfAv">
                        ...
                        <span class="src-mainapp-pages-trackshop-___TrackTable__trackInfoFirstLine___1pleZ css-1sl0tih">
                            ...
                            <span>@foreach($item->associatedModel->artists as $artist){{$artist->name}}@if(!$loop->last), @endif @endforeach</td></span>
                        </span>
                        </div>
                ...
                <div class="src-mainapp-pages-trackshop-___Trackshop__totalSection___1_bz4">
                    <div class="src-mainapp-pages-trackshop-___GrandTotalSection__opacityWrapper___1eQI8">
                        <span class="src-mainapp-pages-trackshop-___GrandTotalSection__subTotal___1_xyL css-1sl0tih">Subtotal<span class="css-1sl0tih">{{ Cart::getSubtotal() }} {{ config('settings.currency', 'USD') }}</span></span>
                    </div>
                </div>
                @if(env('APP_COUPON_PLUGIN'))
                                        ...
                                        <form class="ajax-form" method="post" action="">
                                            <div class="input-group mb-3">
                                                ...
                                                <div class="input-group-append">
                                                    <button class="input-group-text" type="submit" id="basic-addon2">Apply</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
                ...
                @endif
                <div class="d-block">
                    <a class="btn btn-primary btn-block float-right make-payment" data-should-hide-description="true">Purchase</a>
                </div>
            </div>
        @else
