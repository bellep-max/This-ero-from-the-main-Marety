...
<div class="d-flex flex-row w-100 align-items-center justify-content-start gap-3 module p-2 @if(isset($sortable) && $sortable) can-drag drag-handle @else draggable @endif" data-type="song" data-id="{{ $song->id }}" data-index="{{ $index }}" data-plays="{{ $song->plays }}">
    ...
    @endif
    <a class="btn-icon favorite" data-type="song" data-id="{{ $song->id }}">
        @if(auth()->check() && $song->favorite)
    ...
    <div class="position-relative flex-shrink-0" data-toggle="contextmenu" data-trigger="right" data-type="song" data-id="{{ $song->id }}" @if(isset($sortable) && $sortable) data-removable="true" data-removable-type="playlist" data-removable-id="{{ $playlist_id }}" @endif>
        <img class="img-thumb rounded-2" src="{{ $song->artwork_url }}" alt="{{ $song->title }}">
        <div class="row-actions primary song-play-action">
            <a class="btn-play-sm play-object hide" data-type="song" data-id="{{ $song->id }}">
    ...
    <div class="metadata text-truncate" data-toggle="contextmenu" data-trigger="right" data-type="song" data-id="{{ $song->id }}" @if(isset($sortable) && $sortable) data-removable="true" data-removable-type="playlist" data-removable-id="{{ $playlist_id }}" @endif>
        <a class="title color-text font-default fs-12" href="{{ $song->permalink_url }}">{{ $song->title }}</a>
    ...
    <div class="ms-auto d-flex flex-row gap-5 align-items-center">
        <div class="font-merge fs-12 color-grey">{{ humanTime($song->duration) }}</div>
        <a class="btn-icon-only color-text invisible" data-toggle="contextmenu" data-trigger="left" data-type="song" data-id="{{ $song->id }}" @if(isset($sortable) && $sortable) data-removable="true" data-removable-type="playlist" data-removable-id="{{ $playlist_id }}" @endif>
