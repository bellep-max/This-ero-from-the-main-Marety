...
@elseif ($element == "search")
    <div class="module module-row tall podcast" data-index="{{ $index }}">
        <div class="img-container">
            <img class="img" src="{{ $podcast->artwork_url }}" alt="{{ $podcast->title }}">
        </div>
        <div class="metadata podcast">
            <a href="{{ $podcast->permalink_url }}" class="title podcast-link">{{ $podcast->title }}</a>
            <div class="meta-inner">
                @if(isset($podcast->artist)) <span data-translate-text="BY">by</span> <a href="{{$podcast->artist->permalink_url}}" class="meta-text artist-link" title="{{ $podcast->artist->name }}">{{$podcast->artist->name}}</a> @endif
            </div>
        </div>
    </div>
@elseif ($element == "grid")
...
{{--                @if(isset($podcast->artist))--}}
{{--                    <a href="{{ $podcast->artist->permalink_url }}">{{ $podcast->artist->name }}</a>--}}
{{--                @endif--}}
{{--            </div>--}}
{{--        </div>--}}
{{--    </div>--}}
    <div class="podcast-carousel-item" data-index="{{ $index }}">
        <div class="podcast-carousel-item__img position-relative">
            <img class="img" src="{{$podcast->artwork_url}}" alt="{{$podcast->title}}">
            <a class="overlay-link" href="{{ $podcast->permalink_url }}"></a>
            <div class="row-actions primary position-absolute top-50 start-50 translate-middle">
                <a class="btn-default btn-rnd btn-pink play-lg play-object" data-type="podcast" data-id="{{ $podcast->id }}">
        ...
        <div class="podcast-carousel-item__info text-truncate">
            <a href="{{ $podcast->permalink_url }}" class="podcast-carousel-item__info__title">
                {{ $podcast->title }}
            </a>
            <div class="podcast-carousel-item__info__desc">
                @if(isset($podcast->artist))
                    <a href="{{ $podcast->artist->permalink_url }}">{{ $podcast->artist->name }}</a>
                @endif
            </div>
