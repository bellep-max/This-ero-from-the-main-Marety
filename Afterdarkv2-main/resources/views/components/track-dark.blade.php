...
<div class="track no-released sponsored">
    <div class="col-10 order-first col-lg-7 d-flex flex-row align-items-center gap-3">
        ...
        <div class="d-flex flex-column justify-content-center align-items-start text-truncate">
            @if ($adventure)
                <span class="title color-text font-default">
                    {{ $song->title }}
                </span>
            @else
                <a class="title color-text font-default link" href="{{ $song->permalink_url ?? '' }}">
                    {{ $song->title }}
                </a>
            @endif
            <div class="description color-text">
                <div class="d-none d-md-block">
                    <span>by</span>
                    @auth
                        <a href="{{ $song->user->permalink_url ?? '' }}" class="color-text link" title="{{ $song->user->name ?? '' }}">{{ $song->user->name ?? '' }}</a>
                    @elseguest
    ...
    @if ($discover)
        <div class="col-12 order-last order-lg-1 col-lg-4 d-flex flex-row justify-content-start align-items-center flex-wrap gap-2" loop-iteration="{{ $loop->iteration }}">
    ...
    @auth
        <div class="col-2 order-1 order-lg-last col-lg-1 d-flex flex-row align-items-center justify-content-end">
            <a class="btn-icon-only color-text"
                    data-toggle="contextmenu"
                    data-trigger="left"
                    data-type="{{ $adventure ? 'adventure' : 'song' }}"
                    data-id="{{ $song->id }}"
            >
