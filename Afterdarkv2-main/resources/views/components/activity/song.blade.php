@isset($song->title)
    ...
    @if(in_array($element, ["genre", "artist", "collection"]))
        <div data-type="song" data-id="{{ $song->id }}" class="track no-released sponsored ng-star-inserted song">
                    ...
                    @if ($song->user)
                        <div class="musician-container">
                            ...
                            <div class="musician mobile">
                                <span class="ng-star-inserted text-white">by {{ $song->user->name }}</span>
                            </div>
                        </div>
                    @endif
                </div>
        ...
        {{-- <div class="module module-row song tall draggable" data-type="song" data-id="{{  $song->id }}" data-index="{{ $index }}" data-plays="{{ $song->plays }}">
               <div class="fav-btn">
                ...
                <div class="metadata" data-toggle="contextmenu" data-trigger="right" data-type="song" data-id="{{ $song->id }}">
                    <div class="title">
            ...
            <div class="metadata" data-toggle="contextmenu" data-trigger="right" data-type="song" data-id="{{ $song->id }}">
                ...
                <div class="artist">
                    @foreach($song->artists as $artist)
                        <a href="{{ $artist->permalink_url }}" title="{{ $artist->name }}">{{ $artist->name }}</a>
                        @if(!$loop->last), @endif
                    @endforeach
                </div>
                <div class="duration">{{ humanTime($song->duration) }}</div>
    ...
    @elseif ($element == "activity")
        @if ($songCount > 1)
            <a href="{{ $song->permalink_url }}" class="song draggable" data-toggle="contextmenu" data-trigger="right" data-type="song" data-id="{{ $song->id }}">
                <img src="{{ $song->artwork_url }}" class="img-thumb rounded-2" data-toggle="song-popover" data-placement="top" data-html="true" data-target="tmpl-song-popover" data-id="{{ $song->id }}" alt="{{ $song->title }}">
            </a>
        @else
            <div class="d-flex flex-row gap-2 justify-content-start align-items-start" data-toggle="contextmenu" data-trigger="right" data-type="song" data-id="{{ $song->id }}">
                <a href="{{ $song->permalink_url }}" class="song draggable flex-shrink-0" data-type="song" data-id="{{ $song->id }}">
                    <img class="img-thumb rounded-2" src="{{ $song->artwork_url }}" data-id="{{ $song->id }}" data-toggle="song-popover" data-placement="top" data-html="true" data-target="tmpl-song-popover">
                </a>
                <div class="d-flex flex-column justify-content-start gap-2 fs-13">
                    <a class="font-default fw-bolder color-text" href="{{ $song->permalink_url }}">{{ $song->title }}</a>
                    @foreach($song->artists as $artist)
                        <a class="fw-bolder font-default color-pink" href="{{ $artist->permalink_url }}">{{ $artist->name }}</a>
                        @if(!$loop->last), @endif
                    @endforeach
                    <a class="btn-default btn-outline btn-narrow play play-object" data-type="song" data-id="{{ $song->id }}">
        ...
        @endif
    @else
        <div class="swiper-slide module-cell draggable rounded-3" data-toggle="contextmenu" data-trigger="right" data-type="{{ \App\Constants\TypeConstants::SONG }}" data-id="{{ $song->id }}" data-index="{{ $index }}" data-plays="{{ $song->plays }}">
...
{{--                        </div>--}}
{{--                    </div>--}}
                    <a class="btn-default btn-outline btn-rnd play-object" data-type="song" data-id="{{ $song->id }}">
...
{{--                        <svg class="icon-play" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/><path d="M0 0h24v24H0z" fill="none"/></svg>--}}
            ...
            <div class="d-flex flex-column text-start p-2">
                <a class="title font-default color-text" href="{{ $song->permalink_url }}" title="{{ $song->title }}">{{ $song->title }}</a>
                @if($song->artists->isNotEmpty())
                    <span class="byline">
                        <span data-translate-text="BY">{{ __('web.BY') }}</span>
                        @foreach($song->artists as $artist)
                            <a href="{{ $artist->permalink_url }}" class="artist-link artist" title="{{ $artist->name }}">{{ $artist->name }}</a>
                            @if(!$loop->last), @endif
                        @endforeach
                    </span>
                @endif
