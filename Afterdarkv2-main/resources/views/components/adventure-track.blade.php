@php
    $song->children = $song->adventureChildSongs;
@endphp
<script>var song_data_{{ $song->id }} = {!! json_encode($song) !!}</script>
<div data-type="song" data-id="{{ $song->id }}" class="track no-released sponsored ng-star-inserted song w-100">
    <div class="d-flex flex-row w-100 align-items-center justify-content-start">
        <div class="col-play">
            <span class="number">{{ $song->id }}</span>
            <a class="mat-focus-indicator btn-play mat-fab mat-button-base mat-accent ng-star-inserted play-object"
        ...
        <img alt="Track image cover" src="{{ $song->artwork_url }}"
             onerror="this.src = '/common/default/song.png'" class="track-image">
        <div class="track-info">
            ...
            <div class="musician-container">
                <div class="musician desktop">
                    ...
                    <a href="{{ $song->user->permalink_url }}"
                       title="{!! $song->user->name !!}">{!! $song->user->name !!}</a>
                </div>
        ...
        @if ($canEdit)
            <div class="track__action d-flex align-items-center justify-content-end">
                <button class="btn btn-link d-flex" data-toggle="contextmenu" data-trigger="left"
                        data-type="adventure" data-id="{{ $song->id }}">
                    <svg height="512pt" viewBox="-192 0 512 512" width="512pt"
