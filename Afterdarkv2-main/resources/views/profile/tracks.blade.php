...
@section('pagination')
    @include('commons.song', ['songs' => $profile->tracks, 'element' => 'collection'])
@endsection
            ...
            <div class="inner">
                <div class="actions-primary">
                    @if (! auth()->check() || (auth()->check() && auth()->user()->username != $profile->username))
                        <a class="btn-rounded favorite @if($profile->favorite) btn-success @endif" data-id="{{ $profile->id }}" data-type="1" data-title="{{ $profile->name }}" data-url="{{ $profile->url }}">
                    ...
                    @endif
                    <a class="btn-default btn-outline btn-narrow share" data-type="user" data-id="{{ $profile->id }}">
                ...
                <h1 title="{{ $profile->username }}" class="">{{ $profile->name }}<span class="subpage-header"> / {{ __('web.TRACKS') }}</span></h1>
                @if(isset($profile->group->role_id))
                    <div class="byline"><span>{!! $profile->group->role->name !!}</span></div>
                @endif
            </div>
        </div>
        @if (count($profile->tracks))
            <div class="content">
