        ...
        <div class="page-header user main ">
            <div class="img">
                ...
                @if ($profile->activity_privacy || (auth()->check() && auth()->user()->username == $profile->username))
                    <a class="btn edit-profile" href="{{ route('frontend.settings.index') }}">
                        <span data-translate-text="EDIT_PROFILE">Edit Profile</span>
                    </a>
                @endif
            </div>
            <div class="inner">
                <h1 title="{{ $profile->username }}">{{ $profile->name }}@if(isset($profile->group) && isset($profile->group->role_id) && isset($profile->group->role->name) && isset($profile->group->role->permissions))<span class="group-badge basic-tooltip" tooltip="{{ $profile->group->role->name }}">{!! $profile->group->role->permissions['group_badge'] !!}</span>@endif</h1>
                ...
                <ul class="stat-summary">
                    <li><a href="{{ route('frontend.user.favorites', $profile->username) }}"><span id="song-count" class="num">{{ $profile->favorite_count }}</span><span class="label" data-translate-text="SONGS">{{ __('web.SONGS') }}</span></a></li>
                    <li><a href="{{ route('frontend.user.playlists', $profile->username) }}"><span id="playlist-count" class="num">{{ $profile->playlist_count }}</span><span class="label" data-translate-text="PLAYLISTS">{{ __('web.PLAYLISTS') }}</span></a></li>
                    <li><a href="{{ route('frontend.user.followers', $profile->username) }}"><span id="follower-count" class="num">{{ $profile->follower_count }}</span><span class="label" data-translate-text="FOLLOWERS">{{ __('web.FOLLOWERS') }}</span></a></li>
                </ul>
            </div>
        </div>
        <div id="column1" class="">
            @if (! $profile->activity_privacy || (auth()->check() && auth()->user()->username == $profile->username))
                <h2 id="now-playing-header" data-translate-text="NOW_PLAYING_SONG" class="hide small">Now Playing</h2>
                ...
                @if(count($profile->recent))
                    <div class="content">
                        ...
                        <div class="sub-header user-recent-listens">
                            ...
                            <h2 data-translate-text="USER_RECENT_LISTENS">Recent Listens</h2>
                            @if ((auth()->check() && auth()->user()->username == $profile->username))
