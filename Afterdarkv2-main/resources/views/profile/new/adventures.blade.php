        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col col-xl-9">
                    <div class="d-flex flex-column w-100 gap-4 bg-light rounded-5 p-5 h-75">
                        <div class="font-default fs-4">Adventures</div>
                        <div id="grid-toolbar-container" class="d-flex flex-row flex-wrap justify-content-between align-items-center p-2 gap-4 row-gap-2 bg-light rounded-3">
                            <div class="d-flex flex-row justify-content-start align-items-center flex-wrap gap-4 row-gap-2">
                                @if ($hasUploadAccess)
                                    <a class="btn-default btn-outline btn-narrow play-now adventure-init" id="adventure-upload">
                                        <svg height="26" viewBox="0 0 24 24" width="18"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <path d="M8 5v14l11-7z"/>
                                            <path d="M0 0h24v24H0z" fill="none"/>
                                        </svg>
                                        <span>{{ __('Upload Adventure') }}</span>
                                    </a>
                                @endif
                                <div class="d-flex flex-row gap-1">
                                    <a class="btn-default btn-outline btn-narrow play-button play-now" data-target="#songs-grid">
                                        ...
                                        <span>{{ __('web.FEED_PLAY_ALL') }}</span>
                                    </a>
                                    <a class="btn-default btn-outline btn-narrow play-menu" data-target="#songs-grid">
                        ...
                        <div id="songs-grid" class="d-flex flex-column w-100 overflow-y-auto">
                            @foreach ($profile->adventures as $song)
                                @include('frontend.components.track-dark', ['song' => $song, 'discover' => false, 'adventure' => true])
{{--                                @include('frontend.components.adventure-track', ['song' => $song, 'canEdit' => $canEdit])--}}
                            @endforeach
                        </div>
                    </div>
                </div>
