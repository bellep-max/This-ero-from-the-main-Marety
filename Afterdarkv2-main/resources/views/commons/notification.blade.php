@if(count($notifications))
    @foreach ($notifications as $index => $notification)
        @if((isset($notification->details->object) && isset($notification->details->object->permalink_url)) || isset($notification->details->object->object->object->permalink_url))
            <a
                ...
                @elseif (in_array($notification->action, ['commentMusic', 'commentMentioned', 'replyComment']))
                ...
                @elseif (in_array($notification->action, ['inviteCollaboration', 'acceptedCollaboration']))
                ...
                @elseif ($notification->action == 'followUser')
                    href="{{ route('frontend.user.show', $notification->details->host) }}"
            ...
            >
                <div class="module-feed-event {{ $notification->action }} d-flex flex-row justify-content-start align-items-start gap-2">
                        ...
                        <p class="event-action">
                            ...
                            @elseif ($notification->action == 'commentMusic')
                                @switch($notification->notificationable_type)
                                    @case(\App\Models\Song::class)
                                        ...
                                        !!}
                                    @break
                                    @case(\App\Models\Activity::class)
                                        {!! __('web.NOTIFICATION_USER_COMMENT', [
                                                'user' => $notification->details->host->name ?? '',
                                        ...
                                        !!}
                                    @break
                                    @case(\App\Models\Playlist::class)
                                        {!! __('web.NOTIFICATION_USER_COMMENT', [
                                                'user' => $notification->details->host->name,
                                                'object' => trans('web.PLAYLIST_ARTICLE'),
                                            ])
                            ...
                            @elseif ($notification->action == 'followArtist')
                                {!! __(count($notification->details->objects) == 1 ? 'web.FEED2_USER_FOLLOWING' : 'web.FEED2_USER_FOLLOWING', [
                                        'user' => htmlLink($notification->user->name, route('frontend.user.show', ['user' => $notification->user->username]), 'user-link'),
                                ...
                                !!}
                            @elseif ($notification->action == 'followPlaylist')
                                {!! __($notification->details->object->user->id == auth()->id() ? 'web.NOTIFICATION_USER_SUBSCRIBED_YOUR' : 'web.NOTIFICATION_USER_SUBSCRIBED', [
                                        'user' => htmlLink($notification->details->host->name, route('frontend.user.show', ['user' => $notification->details->host->username]), 'user-link'),
                                        'object' => htmlLink($notification->details->object->user->id == auth()->id() ? trans('web.PLAYLIST') : $notification->details->object->title, $notification->details->object->permalink_url, 'playlist-link'),
                                    ])
                                !!}
                            @elseif ($notification->action == 'addSong')
                                @isset($notification->details->model)
                                    {!! __(count($notification->details->objects) == 1 ? 'web.FEED2_ARTIST_UPLOADED' : 'web.FEED2_ARTIST_UPLOADED_MANY', [
                                            'artist' => htmlLink($notification->details->model->name, route('frontend.artist.show', ['artist' => $notification->details->model->id, 'slug' => $notification->details->model->name]), 'user-link'),
                                            'objectCount' => count($notification->details->objects),
                    ...
                    <span class="close-notification ms-auto" data-id="{{ $notification->id }}" data-role="notification">
...
{{--                    <span class="module-time text-secondary">{{ timeElapsedString($notification->created_at) }}</span>--}}
        ...
        @if(isset($notification->details->objects) && isset($notification->details->objects[0]) && isset($notification->details->model))
            <a @if ($notification->action == 'addToPlaylist')
                   href="{{ $notification->details->model->permalink_url }}"
               @elseif ($notification->action == 'addSong')
                   href="{{ route('frontend.artist.show', ['artist' => $notification->details->model->id, 'slug' => $notification->details->model->name]) }}"
               @elseif ($notification->action == 'addEvent')
                   href="{{ route('frontend.artist.events', ['id' => $notification->details->model->id, 'slug' => $notification->details->model->name]) }}"
               @endif
               class="w-100 notification-item border-bottom p-2"
            >
                <div class="module-feed-event {{ $notification->action }} d-flex flex-row justify-content-start align-items-start gap-2">
                    @if (in_array($notification->action, ['addSong', 'addEvent']))
                        <div class="feed-user-image">
                            <img class="author-image-medium" src="{{ $notification->details->model->artwork_url }}">
                        </div>
                    @else
                        <div href="{{ route('frontend.user.show', $notification->user->username) }}" class="feed-user-image">
                    ...
                    <span class="close-notification ms-auto" data-id="{{ $notification->id }}" data-role="activity">
                        <svg width="14" height="14" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512">
                            <!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.-->
                            <path d="M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z"/>
                        </svg>
