@foreach ($users as $index => $user)
    <script>var user_data_{{ $user->id }} = {!! json_encode(['id' => $user->id, 'username' => $user->username, 'name' => $user->name, 'follower_count' => $user->follower_count, 'artwork' => $user->artwork, 'permalink_url' => $user->permalink_url]) !!}</script>
    @if($element == "carousel")
        <div class="module module-cell playlist block swiper-slide draggable" data-toggle="contextmenu" data-trigger="right" data-type="user" data-id="{{ $user->id }}">
            <div class="img-container rounded-circle">
                <img class="img" src="{{ $user->artwork }}" alt="{{ $user->name }}">
