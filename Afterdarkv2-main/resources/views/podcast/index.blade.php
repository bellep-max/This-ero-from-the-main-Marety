    ...
    <div class="bg-gradient-default py-3 p-md-5 p-lg-6 min-vh-100">
        <div class="container d-flex flex-column w-100 gap-4 bg-light rounded-5 p-3 p-lg-5">
            <div class="row gy-3">
                <div class="col-12 col-lg-4 d-flex justify-content-center justify-content-xl-start align-items-start">
                    <img src="{{ $podcast->artwork_url }}" alt="{{ $podcast->title }}"
                         class="rounded-4 border-pink img-fluid">
                </div>
                <div class="col">
                    <div class="d-flex flex-column gap-4">
                        <div class="d-flex flex-row gap-4 justify-content-xl-start justify-content-center align-items-center flex-wrap">
                            @include('frontend.components.circled-text', ['title' => $podcast->episodes()->max('season'), 'description' => 'Seasons'])
                            @include('frontend.components.circled-text', ['title' => $podcast->episodes_count, 'description' => 'Episodes'])
                        </div>
                        <script>var podcast_data_{{ $podcast->id }} = {!! json_encode($podcast->makeHidden('episodes')) !!}</script>
                        <div class="d-flex flex-column align-items-start justify-content-start gap-2 py-3 border-top border-bottom">
                            <span class="font-default fs-5">{{ $podcast->title }}</span>
                            @if (!$podcast->is_visible)
                                <span class="private"
                                      data-translate-text="PRIVATE">{{ __('web.PRIVATE') }}</span>
                            @endif
                            @isset ($podcast->artist)
                                <span class="font-merge fs-14">
                                                Podcast by {{ $podcast->artist->name }}
                                            </span>
                            @endisset
                            <div class="d-flex flex-row justify-content-start align-items-center gap-2">
                                ...
                                <span class="fs-14 font-merge">
                                                {{ $podcast->updated_at->format('d.m.Y') }}
                                            </span>
                            </div>
                            @if ($podcast->description)
                                <div class="d-flex flex-row w-100 font-default justify-content-start align-items-center gap-3">
                        ...
                        <div class="d-flex flex-row justify-content-start align-items-center playlist gap-3">
                            @if(! auth()->check() || auth()->check() && isset($podcast->artist) && auth()->user()->artist_id == $podcast->artist->id)
                                <a class="favorite btn-default btn-outline btn-narrow @if($podcast->favorite) on @endif"
                                   data-type="podcast" data-id="{{ $podcast->id }}"
                                   data-title="{{ $podcast->title }}"
                                   data-url="{{ $podcast->permalink_url }}"
                                   data-text-on="{{ __('web.PLAYLIST_UNSUBSCRIBE') }}"
                                   data-text-off="{{ __('web.PLAYLIST_SUBSCRIBE') }}">
                                    <svg class="off" height="18" viewBox="0 0 24 24" width="18"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/>
                                        <path d="M0 0h24v24H0z" fill="none"/>
                                    </svg>
                                    <svg class="on" height="18" viewBox="0 0 24 24" width="18"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path d="M0 0h24v24H0z" fill="none"/>
                                    ...
                                    @if($podcast->favorite)
                                        <span class="label"
                                              data-translate-text="PLAYLIST_UNSUBSCRIBE">{{ __('web.PLAYLIST_UNSUBSCRIBE') }}</span>
                                    @else
                                        <span class="label"
                                              data-translate-text="PLAYLIST_SUBSCRIBE"> {{ __('web.PLAYLIST_SUBSCRIBE') }} </span>
                                    @endif
                                </a>
                            @endif
                            <a class="share btn-default btn-outline btn-narrow" data-type="podcast"
                               data-id="{{ $podcast->id }}">
                            ...
                            <a class="podcast-report btn-default btn-outline btn-narrow"
                               data-action="report" data-type="podcast" data-id="{{ $podcast->id }}">
            ...
            <div class="episodes-container">
                <div id="songs-grid"
                     class="d-flex flex-column justify-content-start align-items-start gap-2 playlist-container overflow-y-auto"
                     data-type="podcast" data-id="{{ $podcast->id }}">
                    @foreach($podcast->episodes as $index => $episode)
                        @include('frontend.components.podcast-episode-track', ['episodes' => $episode, 'index' => $index, 'element' => 'genre'])
                    @endforeach
                </div>
            ...
            <div class="row">
                <div class="col">
                    @if($podcast->allow_comments)
                        @include('frontend.default.comments.index', ['object' => (Object) ['id' => $podcast->id, 'type' => 'App\Models\Podcast', 'title' => $podcast->title]])
                    @else
