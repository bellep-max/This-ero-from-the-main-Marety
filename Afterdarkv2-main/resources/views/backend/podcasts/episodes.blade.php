...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('backend.podcasts.index') }}">Podcasts</a></li>
        <li class="breadcrumb-item"><a href="{{ route('backend.podcasts.edit', $podcast) }}">{{ $podcast->title }}</a>@if(isset($podcast->artist)) - <a href="{{ route('backend.artists.edit', ['artist' => $podcast->artist->id]) }}" title="{{$podcast->artist->name}}">{{$podcast->artist->name}}</a>@endif</li>
        ...
        <div class="col-lg-12 media-info mb-3 podcast">
            <div class="media mb-3">
                ...
                <div class="media-body">
                    <h5 class="m-0">{{ $podcast->title }}@if(isset($podcast->artist)) - <a href="{{ route('backend.artists.edit', $podcast->artist) }}" title="{{$podcast->artist->name}}">{{$podcast->artist->name}}</a>@endif</h5>
                    <p>Episodes: {{ $podcast->episodes->total() }}</p>
                    <p class="m-0"><a href="{{ $podcast->permalink_url }}" class="btn btn-warning" target="_blank">Preview @if(! $podcast->approved) (only Moderator) @endif</a> <a href="{{ route('backend.podcasts.episodes.upload', $podcast) }}" class="btn btn-success">Upload</a></p>
                </div>
            </div>
        </div>
        <div class="col-lg-12">
            <form id="mass-action-form" method="POST" action="{{ route('backend.podcasts.episodes.batch', $podcast) }}">
                @csrf
