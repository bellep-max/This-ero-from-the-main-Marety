...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('backend.languages.index') }}">Languages</a></li>
                    ...
                    @if(count($translations))
                        <table class="table table-striped">
                            ...
                            <tr>
                                ...
                                <th class="uppercase">{{ trans('langcode.' . env('APP_LOCALE', 'en')) }}</th>
                                @if(env('APP_LOCALE', 'en') != $language)
                            ...
                            @foreach ($translations as $type => $items)
                                @foreach ($items as $group => $translations)
                                    @foreach ($translations as $key => $value)
                                        @if (!is_array($value[env('APP_LOCALE', 'en')]))
                                            <tr>
                                                <td class="text-wrap text-break">{{ $group }}</td>
                                                <td class="text-wrap text-break">{{ $key }}</td>
                                                <td class="lang-editable text-wrap text-break"
                                                    data-locale="{{ env('APP_LOCALE', 'en') }}"
                                                    data-group="{{ $group }}"
                                                    data-key="{{ $key }}"
                                                    data-uri="{{ route('backend.languages.translations.update', env('APP_LOCALE', 'en')) }}"
                                                >{{ $value[env('APP_LOCALE', 'en')] }}</td>
                                                @if(env('APP_LOCALE', 'en') != $language)
                                                    <td class="lang-editable text-wrap text-break"
                                                        data-locale="{{ $language }}"
                                                        data-group="{{ $group }}"
                                                        data-key="{{ $key }}"
                                                        data-uri="{{ route('backend.languages.translations.update', $language) }}"
                                                    >
                                                        {{ $value[$language] }}
                                                    </td>
                                                @endif
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <form method="POST" action="{{ route('backend.languages.translations.create', $language) }}">
                @csrf
