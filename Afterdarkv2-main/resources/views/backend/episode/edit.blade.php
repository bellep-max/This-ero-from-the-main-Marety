...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active"> {{ $song->title }} - @foreach($song->artists as $artist)<a href="{{ route('backend.artists.edit', ['artist' => $artist]) }}" title="{{$artist->name}}">{{$artist->name}}</a>@if(!$loop->last), @endif @endforeach</li>
    </ol>
    <div class="row">
        <div class="col-lg-12 media-info mb-3 album">
            <div class="media mb-3">
                <img class="mr-3" src="{{ $song->artwork }}">
                <div class="media-body">
                    <h5 class="m-0">{{ $song->title }} - @foreach($song->artists as $artist)<a href="{{ route('backend.artists.edit', ['artist' => $artist]) }}" title="{{$artist->name}}">{{$artist->name}}</a>@if(!$loop->last), @endif @endforeach</h5>
                    <h5>
                        @if($song->mp3)
        ...
        <div class="col-lg-12">
            <form role="form" action="" enctype="multipart/form-data" method="post">
                ...
                <a class="btn btn-danger"  href="{{ route('backend.songs.delete', ['songVisible' => $song]) }}" onclick="return confirm('Are you sure want to delete this song?')" data-toggle="tooltip" data-placement="left" title="Delete this song"><i class="fas fa-fw fa-trash"></i></a>
            </form>


            <div class="mt-5 collapse" id="collapseExample">
                <form role="form" method="post" action="{{ route('backend.songs.reject', ['songVisible' => $song]) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label>Comment</label>
