...
@section('content')
    <script>
        ...
        var RevenueSourcesChartData = [
            @foreach($plans as $plan)
                "{{ \App\Models\MESubscription::query()->where('plan_id', $plan->id)->whereIn('subscriptions.status', \App\Constants\StatusConstants::getAll())->count() }}",
            @endforeach
        ]
        var PaymentMethodChartData = [{{ \App\Models\MESubscription::query()->count() }}, {{ \App\Models\MESubscription::query()->count() }}]
    ...
    <div class="col-lg-12 mb-5">
        <form role="form" method="POST" action="{{ route('backend.reports.store') }}">
            @csrf
            <div class="form-group row mb-2 p-0">
                <label class="col-sm-3 col-form-label">From</label>
                <div class="col-sm-9">
                    <input class="form-control" type="datetime-local" name="from" value="{{ old('from', $fromDate ?? now()->subMonth()) }}" required>
                </div>
            </div>
            <div class="form-group row mb-2 p-0">
                <label class="col-sm-3 col-form-label">To</label>
                <div class="col-sm-9">
                    <input class="form-control" type="datetime-local" name="to" value="{{ old('to', $toDate ?? now()) }}" required>
                </div>
            </div>
