        ...
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3 border-0">
                    ...
                    <a href="{{ route('backend.posts.create') }}" class="btn btn-primary btn-sm float-right">Add new article</a>
                </div>
                <div class="card-body p-0">
                    <div class="accordion" id="collapseMetaTags">
                        <div id="collapseOne" class="collapse p-4" aria-labelledby="headingOne" data-parent="#collapseMetaTags">
                            <form class="search-form" action="{{ route('backend.posts.index') }}">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="row">
                                            ...
                                            <div class="col-6 input-group mb-2">
                                                {!! \App\Services\Backend\BackendService::makeDropdown(\App\Constants\SearchConstants::getFullList(\App\Constants\TypeConstants::POST), 'location',  request()->input('location') ? request()->input('location') : 0) !!}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <select multiple="" class="form-control select2-active" name="category[]">
                                            {!! \App\Services\Backend\BackendService::categorySelection(request()->input('category')) !!}
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-control multi-selector-without-sortable" data-ajax--url="{{ route('backend.users.search') }}" name="userIds[]" multiple="">
                                            @if(request()->input('userIds'))
                                                @foreach (\App\Models\User::find(request()->input('userIds')) as $user)
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="form-inline">
                                            {!! \App\Services\Backend\BackendService::makeDropdown(\App\Constants\NewsStatusConstants::getFullList(), 'status',  request()->input('status') ? request()->input('status') : \App\Constants\NewsStatusConstants::CODE_ALL) !!}
                                            <div class="input-group ml-3">
                                                <div class="input-group-prepend">
                                                    <div class="input-group-text">Results/Page</div>
                                                </div>
                                                <input type="number" class="form-control" step="1" name="results_per_page" value="{{ request()->input('results_per_page') ? request()->input('results_per_page') : 50 }}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <a href="{{ route('backend.posts.index') }}" class="btn btn-secondary">Clear</a>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
            <form id="mass-action-form" method="post" action="{{ route('backend.posts.batch') }}">
                @csrf
                <table class="table table-striped">
                    ...
                    @foreach ($posts as $index => $post)
                        <tr>
                            <td>@if($post->fixed) <span class="badge badge-danger badge-pill">fixed</span> @endif<a href="{{ route('backend.posts.edit', $post) }}">{{ $post->title }}</a></td>
                            <td class="desktop" width="200px">@foreach($post->categories as $category)<a href="{{ route('backend.categories.edit', $category) }}" title="{{ $category->name }}">{{$category->name}}</a>@if(!$loop->last), @endif @endforeach</td>
                            ...
                            @endif
                            <td class="desktop">{{ $post->updated_at->diffForHumans() }}</td>
                            ...
                            <td>
                                <a href="{{ route('backend.posts.edit', $post) }}" class="row-button edit"><i class="fas fa-fw fa-edit"></i></a>
                                <a href="{{ route('backend.posts.destroy', $post) }}" class="row-button delete" onclick="return confirm('Are you sure want to delete this post?')"><i class="fas fa-fw fa-trash"></i></a>
                            </td>
                            <td>
                                <label class="engine-checkbox">
