        ...
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3 border-0">
                    ...
                    <a href="{{ route('backend.cities.create') }}" class="btn btn-primary btn-sm float-right">Add new city</a>
                </div>
                <div class="card-body p-0">
                    <div class="accordion" id="collapseMetaTags">
                        <div id="collapseOne" class="collapse p-4" aria-labelledby="headingOne" data-parent="#collapseMetaTags">
                            <form class="search-form" action="{{ route('backend.cities.index') }}">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="form-inline">
                                            <div class="filter-country">
                                                {!! BackendService::makeCountryDropdown('country', 'form-control select2-active', request()->input('country')) !!}
                                            </div>
                                ...
                                <a href="{{ route('backend.cities.index') }}" class="btn btn-danger"><i class="fas fa-eraser"></i> Clear Filter</a>
                            </form>
                        </div>
                    </div>
                </div>
            ...
            <form id="mass-action-form" method="POST" action="">
                <table class="table table-striped">
                    ...
                    @foreach ($cities as $city)
                        <tr>
                            <td class="td-image">
                                <img class="media-object" src="{{ $city->artwork }}" alt="{{ $city->name }}">
                            </td>
                            <td><a href="{{ route('backend.cities.edit', $city) }}">{{ $city->name }}</a></td>
                            <td>{{ $city->district }}</td>
                            <td>
                                @if($city->country?->id)
                                    <a href="{{ route('backend.countries.edit', $city->country) }}">{{ $city->country->name }}</a>
                                @else
                            ...
                            <td>
                                <a href="{{ route('backend.cities.edit', $city) }}" class="row-button edit"><i class="fas fa-fw fa-edit"></i></a>
                                <a href="{{ route('backend.cities.destroy', $city) }}" onclick="return confirm('Are you sure want to delete this country?')" class="row-button delete"><i class="fas fa-fw fa-trash"></i></a>
                            </td>
                            <td>
                                <label class="engine-checkbox">
                        ...
                        <div class="form-inline float-sm-right">
                            <div class="form-group mb-2">
                                <select name="action" class="form-control mr-2">
                                    ...
                                    <option value="comments">Pin City</option>
                                    <option value="not_comments">Unpin City</option>
