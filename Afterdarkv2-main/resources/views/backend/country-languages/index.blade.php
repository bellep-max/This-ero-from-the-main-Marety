        ...
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3 border-0">
                    ...
                    <a href="{{ route('backend.country.languages.create') }}" class="btn btn-primary btn-sm float-right">Add new language</a>
                </div>
                <div class="card-body p-0">
                    <div class="accordion" id="collapseMetaTags">
                        <div id="collapseOne" class="collapse p-4" aria-labelledby="headingOne" data-parent="#collapseMetaTags">
                            <form class="search-form" action="{{ route('backend.country.languages.index') }}">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        {!! BackendService::makeDropdown( $governmentForms, "government_form", null, false) !!}
                                    </div>
                                </div>
                                ...
                                <a href="{{ route('backend.country.languages.index') }}" class="btn btn-danger"><i class="fas fa-eraser"></i> Clear Filter</a>
                            </form>
                        </div>
                    </div>
                </div>
            ...
            <form id="mass-action-form" method="post" action="">
                ...
                <table class="table table-striped">
                    ...
                    @foreach ($languages as $language)
                        <tr>
                            <td><a href="{{ route('backend.country.languages.edit', $language) }}">{{ $language->name }}</a></td>
                            <td><a href="{{ route('backend.countries.edit', $language->country) }}">{{ $language->country->name }}</a></td>
                            <td>{{ isset($language->country->region) ? $language->country->region->name : '' }}</td>
                            <td>
                                @if($language->fixed)
                            ...
                            <td>
                                <a href="{{ route('backend.country.languages.edit', $language) }}" class="row-button edit"><i class="fas fa-fw fa-edit"></i></a>
                                <a href="{{ route('backend.country.languages.destroy', $language) }}" onclick="return confirm('Are you sure want to delete this country?')" class="row-button delete"><i class="fas fa-fw fa-trash"></i></a>
                            </td>
                            <td>
                                <label class="engine-checkbox">
