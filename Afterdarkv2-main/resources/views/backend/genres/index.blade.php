...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('backend.genres.create') }}">Add new genre</a>
            ...
            <form method="get" action="">
                <div class="input-group mb-3">
                    ...
                    <div class="input-group-append">
                        <a href="{{ route('backend.genres.index') }}" class="btn btn-warning" id="button-addon1">Clear Filter</a>
                        <button class="btn btn-info" type="submit" id="button-addon2">Search</button>
                    </div>
                </div>
            </form>
            <form method="post" action="{{ route('backend.genres.sort') }}">
                @csrf
                <table class="mt-4 table table-striped table-sortable">
                    ...
                    @foreach ($genres as $genre)
                        <tr>
                            ...
                            <td><img src="{{ $genre->artwork }}" alt="{{ $genre->name }}"></td>
                            <td><a href="{{ route('backend.genres.edit', $genre) }}">{{ $genre->name }}</a></td>
                            <td class="desktop">
                                @if($genre->discover)
                            ...
                            <td>
                                <a href="{{ route('backend.genres.edit', $genre) }}" class="row-button edit"><i class="fas fa-fw fa-edit"></i></a>
                                <a href="{{ route('backend.genres.destroy', $genre) }}" onclick="return confirm('Are you sure?')" class="row-button delete"><i class="fas fa-fw fa-trash"></i></a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
