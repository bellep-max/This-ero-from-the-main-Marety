    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3 border-0">
                    <button class="btn btn-link p-0 m-0" type="button" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                        <span class="m-0 font-weight-bold text-primary">Add New Page</span>
                    </button>
                </div>
                            ...
                            <form role="form" method="post" action="" enctype="multipart/form-data">
                                ...
                                <div class="form-group row">
                                    <label class="col-sm-4">URL
                                        <span class="small">Specify URL of the website for which you want to assign meta tags.</span>
                                    </label>
                                    <div class="col-sm-8">
                                ...
                                <div class="form-group row">
                                    <label class="col-sm-4">Page description
                                        <span class="small">Text that can be displayed on the search engine.</span>
                                    </label>
                                    <div class="col-sm-8">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        {!! \App\Services\Backend\BackendService::makeTagSelector('keywords[]', isset($metatag) && ! old('keywords') ? $metatag->meta_keywords : old('keywords')) !!}
                                    </div>
                                </div>
                                <div class="form-group row border-bottom">
                                    <label class="col-sm-4">Auto general keywords
                                        <span class="small">Automatically mass generate keywords (base on title and description) to maximise your search engine presence. .</span>
                                    </label>
                                    <div class="col-sm-8 col-9">
                                        <label class="switch">
                                            {!! \App\Services\Backend\BackendService::makeCheckbox('auto_keyword', 0) !!}
                                ...
                                <div class="form-group row">
                                    <label class="col-sm-4">Page Artwork
                                        <span class="small mt-2">This will use for social share, if the page already contain an artwork this will overwrite the current one. For example song page will use this instead of song artwork.</span>
                                    </label>
                                    <div class="col-sm-8">
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('backend.metatags.sort') }}">
                        @csrf
                        <table class="table table-striped table-sortable">
                            ...
                            @foreach ($metatags as $index => $metatag)
                                <tr data-id="{{ $metatag->id }}">
                                    ...
                                    <td><img src="{{ $metatag->artwork }}"/></td>

                                    <td>{{ $metatag->url }}</td>
                                    <td><span title="{{ url($metatag->url) }}">{{ url($metatag->url) }}</span></td>
                                    <td>{{ $metatag->info }}</td>
                                    <td>
                                        <a href="{{ route('backend.metatags.edit', $metatag) }}" class="row-button edit"><i class="fas fa-fw fa-edit"></i></a>
                                        <a href="{{ route('backend.metatags.destroy', $metatag) }}" onclick="return confirm('Are you sure?')" class="row-button delete"><i class="fas fa-fw fa-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
