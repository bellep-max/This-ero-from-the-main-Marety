...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12">
            <form method="GET" action="{{ route('backend.withdraws.index') }}">
            ...
            <table class="table table-striped datatables table-hover">
                ...
                @foreach ($withdraws as $index => $withdraw)
                    @if ($withdraw->user)
                        <tr>
                            <td><a href="{{ route('backend.users.edit', $withdraw->user) }}">{{ $withdraw->user->name }}</a></td>
                            ...
                            <td>
                                @if ($withdraw->user->payment_method == \App\Constants\PaymentMethodConstants::PAYPAL)
                                    {{ $withdraw->user->payment_paypal  }}
                                @else
                                    {{ $withdraw->user->payment_bank  }}
                                @endif
                            </td>
                            <td>{{ $withdraw->created_at->format('M j, Y') }}</td>
                            <td>
                                @if ($withdraw->paid)
