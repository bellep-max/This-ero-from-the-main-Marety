...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('backend.albums.edit', ['albumVisible' => $album]) }}">{{ $album->title }}</a> - @foreach($album->artists as $artist)<a href="{{ route('backend.artists.edit', ['artist' => $artist]) }}" title="{{$artist->name}}">{{$artist->name}}</a>@if(!$loop->last), @endif @endforeach</li>
        <li class="breadcrumb-item active">Tracks list</li>
    </ol>
    <div class="row">
        <div class="col-lg-12 media-info mb-3 album">
            <div class="media mb-3">
                <img class="mr-3" src="{{ $album->artwork }}">
                <div class="media-body">
                    <h5 class="m-0">{{ $album->title }} - @foreach($album->artists as $artist)<a href="{{ route('backend.artists.edit', ['artist' => $artist]) }}" title="{{$artist->name}}">{{$artist->name}}</a>@if(!$loop->last), @endif @endforeach</h5>
                    <p>Songs: {{ $album->song_count }}</p>
                    <p class="m-0"><a href="{{ $album->permalink_url }}" class="btn btn-warning" target="_blank">Preview @if(! $album->approved) (only Moderator) @endif</a></p>
                </div>
            </div>
        </div>
        <div class="col-lg-12">
            ...
            <form id="mass-action-form" method="post" action="{{ route('backend.albums.tracklist.batch', ['albumVisible' => $album]) }}">
                @csrf
