...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('backend.albums.index') }}">Albums</a></li>
        <li class="breadcrumb-item active"><a href="{{ route('backend.albums.edit', $album) }}">{{ $album->title }}</a> - @foreach($album->artists as $artist)<a href="{{ route('backend.artists.edit', $artist) }}" title="{{$artist->name}}">{{$artist->name}}</a>@if(!$loop->last), @endif @endforeach</li>
    </ol>
    <div class="row">
        <div class="col-lg-12 media-info mb-3 album">
            <div class="media mb-3">
                <img class="mr-3" src="{{ $album->artwork }}" alt="{{ $album->title }}">
                <div class="media-body">
                    <h5 class="m-0">{{ $album->title }} - @foreach($album->artists as $artist)<a href="{{ route('backend.artists.edit', $artist) }}" title="{{$artist->name}}">{{$artist->name}}</a>@if(!$loop->last), @endif @endforeach</h5>
                    <p>Songs: {{ $album->song_test_count }}</p>
                    <p class="m-0"><a href="{{ $album->permalink_url }}" class="btn btn-warning" target="_blank">Preview @if(! $album->approved) (only Moderator) @endif</a> <a href="{{ route('backend.albums.tracklist', $album) }}" class="btn btn-info">Tracks List</a></p>
                </div>
            </div>
        </div>
        <div class="col-lg-12">
            ...
            <form role="form" method="post" action="" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <div class="col-sm-12">
                        <label class="switch">
                            {!! BackendService::makeCheckbox('update-song-artwork') !!}
                ...
                <div class="form-group">
                    ...
                    <select multiple="" class="form-control select2-active" name="genre[]">
                        {!! BackendService::genreSelection($albumGenres) !!}
                    </select>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <div class="col-sm-12">
                        <label class="switch">
                            {!! BackendService::makeCheckbox('approved', $album->approved ) !!}
                            <span class="slider round"></span>
                        </label>
            ...
            <div class="mt-5 collapse" id="collapseExample">
                <form role="form" method="post" action="{{ route('backend.albums.reject', $album) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label>Comment</label>
