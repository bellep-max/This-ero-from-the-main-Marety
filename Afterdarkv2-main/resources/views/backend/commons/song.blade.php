@foreach ($songs as $index => $song)
    <tr>
        ...
        @endif
        <td class="td-image">
            <a href="{{ route('backend.songs.edit', $song) }}" data-toggle="tooltip" data-placement="left" title="Edit this song">
        ...
        <td class="desktop">
            @foreach ($song->artists as $artist)
                <a href="{{ route('backend.artists.edit', $artist) }}" title="{$artist.name}">
                    {{ $artist->name }}
                </a>
                @if(!$loop->last), @endif
            @endforeach
        ...
        @if ($song->album)
            <td class="desktop">
                <a href="{{ route('backend.albums.edit', $song->album) }}">{{ $song->album->title }}</a>
            </td>
        @else
            <td></td>
        @endif
        ...
        <td class="desktop">
            <a class="row-button edit" href="{{ route('backend.songs.edit', $song) }}" data-toggle="tooltip" data-placement="left" title="Edit this song"><i class="fas fa-fw fa-edit"></i></a>
            <a class="row-button delete" href="{{ route('backend.songs.destroy', $song) }}" onclick="return confirm('Are you sure want to delete this song?')" data-toggle="tooltip" data-placement="left" title="Delete this song"><i class="fas fa-fw fa-trash"></i></a>
        </td>
        <td>
