...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active"><a href="{{ route('backend.playlists.index') }}">Playlists</a></li>
        <li class="breadcrumb-item active">{{ $playlist->title }}</li>
    </ol>
    <div class="row">
        <div class="col-lg-12">
            <div class="box-suggest">
                <form method="GET" id="suggest-form" data-object-type="playlist" data-object-id="{{ $playlist->id }}" action="{{ route('backend.songs.search') }}">
            ...
            <form id="mass-action-form" method="POST" action="{{ route('backend.playlists.tracks.batch', $playlist) }}">
                @csrf
