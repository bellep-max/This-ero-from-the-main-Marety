@extends('index')
@section('content')
    @include('podcast.channel.nav', $channel)
    ...
    <div class="container">
        ...
        <div class="page-header channel main medium">
            <div class="img">
                <img src="{{ $channel->artwork_url }}" alt="{{ $channel->title}}">
                @if (auth()->check() && auth()->id() == $channel->user->id)
            ...
            <div class="inner">
                ...
                @if(!$channel->is_visible)<span class="private" data-translate-text="PRIVATE">{{ __('web.PRIVATE') }}</span>@endif
                <div class="byline">
                    <span>Podcast by <a href="{{ $channel->user->permalink_url }}" class="user-link">@{{ $channel->user->name }}</a></span> • <span><span data-translate-text="UPDATED">{{ __('web.UPDATED') }}:</span> {{ \App\Helpers\Helper::timeElapsedString($channel->updated_at) }}</span>
                </div>
        ...
        <div id="column1">
            ...
            <div id="songs-grid" class="@if(auth()->check() && auth()->id() == $channel->user->id) channel-owner sortable @endif" data-type="channel" data-id="{{ $channel->id }}">
