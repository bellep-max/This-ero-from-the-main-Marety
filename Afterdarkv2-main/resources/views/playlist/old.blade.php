    ...
    <div class="container">
        ...
        <div class="page-header playlist main medium">
            ...
            <div class="inner">
                ...
                @if(!$playlist->is_visible)<span class="private" data-translate-text="PRIVATE">{{ __('web.PRIVATE') }}</span>@endif
                <div class="byline">
                    @if(isset($playlist->user)) <span>Playlist by <a href="{{ $playlist->user->permalink_url }}" class="user-link">{{ $playlist->user->name }}</a></span> • <span>@endif<span data-translate-text="UPDATED">{{ __('web.UPDATED') }}:</span> {{ \App\Helpers\Helper::timeElapsedString($playlist->updated_at) }}</span>
                </div>
        ...
        <div id="column1">
            ...
            <div id="songs-grid" class="@if(auth()->check() && auth()->id() == $playlist->user->id) playlist-owner sortable @endif" data-type="playlist" data-id="{{ $playlist->id }}">
                @include('commons.song', ['songs' => $playlist->songs, 'playlist' => $playlist, 'element' => 'playlist', 'sortable' => auth()->check() && auth()->id() == $playlist->user->id ? true : false])
            </div>
        </div>
        <div id="column2">
            <div class="content">
            ...
            @if(isset($playlist->related) && count($playlist->related))
                <div class="content">
                    ...
                    <div id="playlist_digest">
                        <h3 data-translate-text="OTHER_PLAYLISTS">Other Playlists</h3>
                        <ul class="snapshot">
