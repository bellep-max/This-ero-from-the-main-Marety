...
@section('content')
    ...
    <div class="container">
        ...
        <div class="page-header album main medium ">
            <div class="img"> <img id="page-cover-art" src="{{ $album->artwork }}" alt="{{ $album->title }}"> </div>
            <div class="inner">
                <h1 title="{{ $album->title }}">{{ $album->title }}</h1>
                @if(!$album->is_visible)<span class="private" data-translate-text="PRIVATE">{{ __('web.PRIVATE') }}</span>@endif
                <div class="byline">
