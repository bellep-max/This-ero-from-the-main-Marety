                ...
                <div class="col">
                    <div class="container d-flex flex-column w-100 gap-4 bg-light rounded-5 p-3 p-lg-5">
                                ...
                                <div class="d-flex flex-column gap-4">
                                    @auth
                                        <div class="d-flex flex-row gap-4 justify-content-xl-start justify-content-center align-items-center flex-wrap">
                                            @include('frontend.components.circled-text', ['title' => $profile->song_count, 'description' => 'Songs'])
                                    ...
                                    <div class="d-flex flex-row align-items-center py-3 border-top border-bottom gap-3">
                                        <a class="mat-focus-indicator btn-white rounded-circle mat-fab mat-button-base mat-accent ng-star-inserted play-object"
                                           style="filter: drop-shadow(0 0px 5px #DEDEDE)" data-type="song"
                                           data-id="{{ $song->id }}">
                                        ...
                                        <div class="d-flex flex-column align-items-start justify-content-start gap-2">
                                            <span class="font-default fs-4">{{ $song->title }}</span>
                                            <span class="font-merge fs-14">{{ $song->user->name }}</span>
                                            <div class="d-flex flex-row justify-content-start align-items-center gap-2">
                                                ...
                                                <span style="font-size: 14px; font-family: 'Merge One', sans-serif">
                                    ...
                                    @if ($song->tags->isNotEmpty())
                                        <div class="d-flex flex-row gap-2 border-bottom py-3">
                                            @foreach($song->tags as $tag)
                                                <a href="{{ route('frontend.discover.new', ['tags[]' => $tag->tag]) }}"
                                                   class="btn-default btn-outline btn-narrow">{{ $tag->tag }}</a>
                                            @endforeach
                                        </div>
                                    @endif
                                    <div class="d-flex flex-row justify-content-start align-items-center gap-2">
                                        <div class="btn-group">
                                            <a class="add-song btn-default btn-outline btn-narrow" data-type="song"
                                               data-id="{{ $song->id }}">
                                                <svg width="13" height="15" viewBox="0 0 13 15" fill="none"
                                                     xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M12.0714 8.96631H7.42857V13.9663C7.42857 14.5163 7.01071 14.9663 6.5 14.9663C5.98929 14.9663 5.57143 14.5163 5.57143 13.9663V8.96631H0.928571C0.417857 8.96631 0 8.51631 0 7.96631C0 7.41631 0.417857 6.96631 0.928571 6.96631H5.57143V1.96631C5.57143 1.41631 5.98929 0.966309 6.5 0.966309C7.01071 0.966309 7.42857 1.41631 7.42857 1.96631V6.96631H12.0714C12.5821 6.96631 13 7.41631 13 7.96631C13 8.51631 12.5821 8.96631 12.0714 8.96631Z"
                                                    />
                                                </svg>
                                                <span data-translate-text="ADD_SONG">{{ __('web.ADD_SONG') }}</span>
                                            </a>
                                            <a class="add-song btn-default btn-outline btn-rnd btn-narrow" data-type="song"
                                               data-id="{{ $song->id }}">
                                                <span class="caret"></span>
                                            </a>
                                        </div>
                                        <a class="share btn-default btn-outline btn-narrow"
                                           data-type="song" data-id="{{ $song->id }}">
                                    ...
                                    @if (auth()->check() && auth()->id() !== $song->user_id)
                                        @if ($activeUserSubscription)
                                            @if ($activeUserSubscription->status === 'suspended')
                                                <form role="form" action="{{ "/user-subscription/$song->user_id/activate" }}"
                                                      method="post">
                                                    <button type="submit"
                                            ...
                                            @else
                                                <form role="form" action="{{ "/user-subscription/$song->user_id/suspend" }}"
                                                      method="post">
                                                    <button type="submit"
                                                            class="btn-default btn-outline btn-narrow"
                                                            data-type="song" data-id="{{ $song->id }}">
                                            ...
                                            @endif
                                            <form role="form" action="{{ "/user-subscription/$song->user_id/cancel" }}"
                                                  method="post">
                                                <button type="submit"
                                                        class="btn-default btn-outline btn-narrow"
                                                        data-type="song" data-id="{{ $song->id }}">
                                        ...
                                        @else
                                            <form role="form" action="{{ "/user-subscription/$song->user_id/checkout" }}"
                                                  method="post">
                                                <button type="submit"
                                                        class="btn-default btn-outline btn-narrow"
                                                        data-type="song" data-id="{{ $song->id }}">
                        ...
                        @if($song->allow_comments)
                            <div class="row">
                                <div class="col">
                                    @include('frontend.default.comments.index', ['object' => (Object) ['id' => $song->id, 'type' => 'App\Models\Song', 'title' => $song->title]])
                                </div>
                            </div>
                        @endif
                    </div>
