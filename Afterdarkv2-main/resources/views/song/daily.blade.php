...
@section('pagination')
    @include('commons.song', ['songs' => $songs->pluck('historyable'), 'element' => 'collection'])
@endsection
    ...
    <div class="container">
        <div class="page-header tag-header small desktop">
            <div class="inner">
                ...
                {{-- <h1><span title="{{ __('web.GENDER_TAGS')[$vocal] }}">{{ __('web.GENDER_TAGS')[$vocal] }}</span></h1> --}}
                <h1><span title="Top Played Daily">Top Played Daily</span></h1>
                {{-- <div class="byline"><a href="{{ route('frontend.discover.new') }}"><span data-translate-text="SEE_ALL_MOODS">{{ __('web.SEE_ALL_MOODS') }}</span></a></div> --}}
            </div>
        </div>
