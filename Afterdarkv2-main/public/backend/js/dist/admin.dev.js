...
(function ($) {
  ...
  $('#suggest-form').ajaxForm({
    success: function success(response, textStatus, xhr, $form) {
      ...
      $(".add-to").click(function () {
        ...
        $.post('/admin/auth/addSong', {
          ...
          var html = '<tr id="row-' + song.id + '">\n' + '                    <td class="td-image">\n' + '                        <div class="play" data-id="' + song.id + '" data-type="song"><img src="' + song.artwork_url + '"></div>\n' + '                    </td>\n' + '                    <td id="track_' + song.id + '" class="editable" title="Click to edit">' + song.title + '</td>\n' + '                    <td>' + song.artists.map(function (artist) {
            return artist.name;
          }).join(", ") + '</td>\n' + '<td>' + song.album === null ? song.album.title : '' + '</td>' + '<td>' + song.loves + '</td>' + '<td>' + song.plays + '</td>' + '<td>' + '<a class="row-button edit" href="/admin/songs/edit/' + song.id + '" data-toggle="tooltip" data-placement="left" title="Edit this song"></a>' + '<a data-id="' + object_id + '" data-song-id="' + song.id + '" class="row-button remove" data-type="' + object_type + '" data-toggle="tooltip" data-placement="left" title="" data-original-title="Remove from ' + object_type + '"></a></td>\n' + '</tr>';
  ...
  function formatRepo(repo) {
    ...
    var markup = "<div class='select2-result-repository clearfix'>" + "<div class='select2-result-repository__avatar'><img src='" + repo.artwork_url + "' /></div>" + "<div class='select2-result-repository__meta'>" + "<div class='select2-result-repository__title'>" + text + "</div></div></div>";
    return markup;
  }
  ...
  function formatRepoSelection(repo) {
    ...
    var markup = "<div class='select2-result-repository clearfix'>" + "<div class='select2-result-repository__avatar'><img src='" + artwork_url + "' /></div>" + "<div class='select2-result-repository__meta'>" + "<div class='select2-result-repository__title white'>" + text + "</div></div></div>";
  ...
  $('.slide-show-type').on("select2:select", function (e) {
    var type = $(this).val();
    $(".slide-show-selector").addClass('d-none'); //$(".slide-show-selector").find('select').attr('name', 'removed');

    $(".slide-show-selector").find('select').empty();
    $(".slide-show-selector[data-type='" + type + "']").removeClass('d-none'); //$(".slide-show-selector[data-type='" + type + "']").find('select').attr('name', 'object_ids[]');
  });
  ...
  $(function () {
    $('.use_album_cover').change(function () {
      ...
      $.post('/admin/ajax.php?do=songs', {
        ...
        album_id: album_id,
        subaction: subaction
      }, function (data) {//msg_box("success", "Saved!!!")
      });
    });
  });
  ...
  var selectAlllickHandler = function selectAlllickHandler(a, b, c) {
  ...
  $('.filter-country-select').on('change', function () {
    ...
    if (country_code) {
      $.post('/admin/stations/city-by-country-code', {
      ...
      $.post('/admin/stations/language-by-country-code', {
        countryCode: country_code
      }, function (data) {
  ...
  try {
    var getParamCount = function getParamCount() {
      return $('.param').length;
    };

    var appendParameters = function appendParameters(params) {
    ...
    var contentEditor = new JSONEditor(document.getElementById('jsonEditorContent'), {
      mode: 'view'
    });
    var responseHeadersEditor = new JSONEditor(document.getElementById('jsonEditorResponseHeaders'), {
      mode: 'view'
    });
    var jsonEditorRequestHeaders = new JSONEditor(document.getElementById('jsonEditorRequestHeaders'), {
      mode: 'view'
    });
    var timer;
          ...
          if (_typeof(data) === 'object') {
            if (data.status) {
              ...
              if (data.data.original.contentType === 'application/json') {
                ...
                jsonEditorRequestHeaders.set(Object.assign(data.request_headers, {
...
$(function () {
  ...
  var send = function send() {
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        $('#terminal-box').append('<div class="item"><small class="label label-default"> > artisan ' + $input.val() + '<\/small><\/div>').append('<div class="item">' + response + '<\/div>');
        $(".output-body").animate({
          scrollTop: $('.output-body').prop("scrollHeight")
        }, 1000);
        $input.val('');
        isRunning = false;
      }
  ...
  $(document).ready(function () {
    if ($('textarea.post.editor').length) {
      ...
      tinymce.init({
        ...
        plugins: ['advlist autolink lists link image charmap print preview hr anchor pagebreak', 'searchreplace wordcount visualblocks visualchars code fullscreen', 'insertdatetime media nonbreaking save table contextmenu directionality', 'emoticons template textcolor colorpicker textpattern imagetools codesample toc filemanager responsivefilemanager'],
  ...
  $(document).ready(function () {
    if ($('.media-manager').length) {
      /** Create iCheck event */
      var reinventEvent = function reinventEvent() {
  ...
  function animateValue(obj) {
    ...
    if (obj) {
      var run = function run() {
        ...
        var value = Math.round(end - remaining * range); // replace numeric digits only in the original string
      ...
      var textStarting = obj.innerHTML; // remove non-numeric from starting text if not specified
  ...
  $(document).ready(function () {
    if ($('.universal-report').length) {
      ...
      animateValue(document.getElementById('increase-number-6')); //Pie chart
