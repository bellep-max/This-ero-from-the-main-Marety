...
class CronSchedule
{
    ...
    //
    final public function next($time = false): false|array
    {
        // Convert parameters to array datetime
        $arrDT = $this->dtFromParameters($time);
        while (1) {
    ...
    final public function nextAsString($time = false): false|string
    {
        return $this->dtAsString($this->next($time));
    }

    final public function nextAsTime($time = false): false|int
    {
    ...
    //
    private function getEarliestItem($arrItems, $afterItem = false, $allowOverflow = true): false|int|string|null
    {
    ...
    //
    final public function previous($time = false): false|array
    {
        // Convert parameters to array datetime
        $arrDT = $this->dtFromParameters($time);
        while (1) {
    ...
    final public function previousAsString($time = false): false|string
    {
        return $this->dtAsString($this->previous($time));
    }

    final public function previousAsTime($time = false): false|int
    {
    ...
    //
    private function getLatestItem($arrItems, $beforeItem = false, $allowOverflow = true): false|int|string|null
    {
    ...
    final public function asNaturalLanguage(): string
    {
        ...
        $txtDate = match ($classDaysOfMonth . $classMonths) {
            '00' => '',
            default => ' ' . $txtDaysOfMonth[$classDaysOfMonth] . ' ' . $txtMonths[$classMonths],
        };
